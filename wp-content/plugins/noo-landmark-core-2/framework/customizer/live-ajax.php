...
if( !function_exists( 'noo_landmark_func_cusomizer_upload_settings' ) ) :
	function noo_landmark_func_cusomizer_upload_settings(){
		...
		if ( $file_ext == 'json' ) {
			...
			$file = noo_landmark_func_handle_upload_file($file);
...
add_action( 'wp_ajax_noo_landmark_func_cusomizer_upload_settings', 'noo_landmark_func_cusomizer_upload_settings' );
...
function noo_ajax_get_menu(){
	...
	<?php if ( has_nav_menu( $menu_location ) ) :
		...
		echo '<ul class="noo-menu"><li><a href="' . home_url( '/' ) . 'wp-admin/nav-menus.php">' . esc_html__( 'Assign a menu', 'noo-landmark-core' ) . '</a></li></ul>';
	endif; ?>
...
function noo_ajax_get_social_icons(){
	...
	noo_landmark_func_social_icons();
	exit();
}
