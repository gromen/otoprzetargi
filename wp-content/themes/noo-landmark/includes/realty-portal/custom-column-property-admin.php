<?php
if ( ! function_exists( 'noo_support_add_table_columns_property' ) ) :
	 ...
	 * @return mixed|void
	 */
	function noo_support_add_table_columns_property( $columns ) {
		...
		$columns[ 'rp_offer' ]    = esc_html__( 'Offer', 'noo-landmark' );
		$columns[ 'rp_id' ]       = esc_html__( 'ID', 'noo-landmark' );
		$columns[ 'rp_property' ] = esc_html__( 'Property', 'noo-landmark' );
		$columns[ 'rp_price' ]    = esc_html__( 'Price', 'noo-landmark' );
		$columns[ 'rp_posted' ]   = esc_html__( 'Posted', 'noo-landmark' );
		$columns[ 'rp_status' ]   = esc_html__( 'Status', 'noo-landmark' );
		$columns[ 'rp_actions' ]  = esc_html__( 'Actions', 'noo-landmark' );

		return apply_filters( 'rp_admin_property_columns', $columns );
	}

	add_filter( 'manage_edit-noo_property_columns', 'noo_support_add_table_columns_property' );

endif;

if ( ! function_exists( 'noo_support_show_table_columns_property' ) ) :
	 ...
	 * @param $column_name
	 */
	function noo_support_show_table_columns_property( $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'rp_property' :
				...
				if ( $post->post_status !== 'trash' && current_user_can( 'edit_listing', $post->ID ) ) {
					echo '<a href="' . admin_url( 'post.php?post=' . $post->ID . '&action=edit' ) . '" class="tips" data-tip="' . __( 'Edit', 'noo-landmark' ) . '">' . $post->post_title . '</a>';
				} else {
				...
				if ( $post->post_status !== 'trash' ) {
					...
					if ( $property->is_featured() ) {
						echo ' <span class="rp-featured">&dash; ' . __( 'Featured', 'noo-landmark' ) . '</span>';
					}
				}
				...
				if ( current_user_can( 'edit_listing', $post->ID ) ) {
					...
					if ( $post->post_status !== 'trash' ) {
						...
						$admin_actions_listing_title[ 'edit' ] = array(
							...
							'name' => __( 'Edit', 'noo-landmark' ),
							'url'  => get_edit_post_link( $post->ID ),
						);

						$admin_actions_listing_title[ 'view' ] = array(
							'icon'   => 'rp-icon-eye',
							'name'   => __( 'View', 'noo-landmark' ),
				...
				foreach ( $admin_actions_listing_title as $action ) {
					printf( '<a class="button tips" href="%2$s" data-balloon="%3$s" data-balloon-pos="up" target="%5$s"><i class="%1$s"></i></a>', $action[ 'icon' ], esc_url( $action[ 'url' ] ), esc_attr( $action[ 'name' ] ), esc_html( $action[ 'name' ] ), isset( $action[ 'target' ] ) ? esc_html( $action[ 'target' ] ) : false );
				}
			...
			case 'rp_posted' :
				...
				echo '<span class="rp-agent">' . ( empty( $property->agent_info( 'name' ) ) ? __( 'by a guest', 'noo-landmark' ) : sprintf( __( 'by %s', 'noo-landmark' ), $agent ) ) . '</span>';
				break;
			...
			case 'rp_actions':
				...
				if ( 'pending' == $post->post_status && current_user_can( 'publish_post', $post->ID ) ) {
					...
					$admin_actions[ 'approve' ] = array(
						...
						'name' => esc_html__( 'Approve', 'noo-landmark' ),
						'url'  => $url,
					);
				}
				if ( $post->post_status !== 'trash' ) {
					...
					$admin_actions[ 'featured' ] = array(
						'name' => esc_html__( 'Featured', 'noo-landmark' ),
						'url'  => $url_featured,
						'icon' => $icon_featured,
					);
				...
				if ( 0 == $i && $post->post_status == 'publish' ) {
					printf( '<a class="button tips" href="%2$s" data-balloon="%3$s" data-balloon-pos="up" target="%5$s"><i class="rp-icon-ion-android-open"></i></a>', 'view', esc_url( get_permalink( $post->ID ) ), esc_attr( __( 'View', 'noo-landmark' ) ), esc_html( __( 'View', 'noo-landmark' ) ), '_blank' );
				}
	...
	add_action( 'manage_noo_property_posts_custom_column', 'noo_support_show_table_columns_property' );

endif;

if ( ! function_exists( 'noo_support_set_featured_property' ) ) :

	/**
	 * Set featured property
	 *
	 */
	function noo_support_set_featured_property() {

		if ( isset( $_GET[ 'action' ] ) && 'rp_property_feature' == $_GET[ 'action' ] ) {
			if ( ! current_user_can( 'edit_posts' ) ) {
				wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'noo-landmark' ), '', array( 'response' => 403 ) );
			}

			if ( ! check_admin_referer( 'rp-property-feature' ) ) {
				wp_die( esc_html__( 'You have taken too long. Please go back and retry.', 'noo-landmark' ), '', array( 'response' => 403 ) );
			}
	...
	add_action( 'admin_init', 'noo_support_set_featured_property' );

endif;

if ( ! function_exists( 'noo_support_admin_property_approve_action' ) ) :

	/**
	 * Process event approve property
	 */
	function noo_support_admin_property_approve_action() {

		if ( isset( $_GET[ 'action' ] ) && 'rp_property_approve' == $_GET[ 'action' ] ) {
			if ( ! current_user_can( 'edit_posts' ) ) {
				wp_die( __( 'You do not have sufficient permissions to access this page.', 'noo-landmark' ), '', array( 'response' => 403 ) );
			}

			if ( ! check_admin_referer( 'rp-property-approve' ) ) {
				wp_die( __( 'You have taken too long. Please go back and retry.', 'noo-landmark' ), '', array( 'response' => 403 ) );
			}
	...
	add_action( 'admin_init', 'noo_support_admin_property_approve_action' );
endif;